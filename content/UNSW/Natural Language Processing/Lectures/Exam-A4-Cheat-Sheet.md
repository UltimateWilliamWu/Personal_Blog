...
- **Decoder mask pseudocode**: $X \leftarrow X+\mathrm{MHAttention}(X\mid W_l^{dec}, \mathrm{Mask}[t,t']=[[t\le t']])$ means residual update using **masked multi-head attention** over the sequence generated so far.
...
## 中文理解批注
- **One-hot vs embedding**：one-hot 只是“词的编号”，embedding 才是“词的可学习语义位置”。
- **Embedding lookup**：$xW$ 看起来是乘法，本质是从 $W$ 里取出当前 token 那一行。
- **Softmax vs loss**：softmax 负责把分数变概率；cross-entropy 负责惩罚正确答案概率太低。
- **Word2Vec**：不是人工词典，而是用“真实上下文 vs 假上下文”的训练信号把词向量慢慢调出来。
- **Negative sampling**：不再让模型从全词表里选一个词，而是判断“这对词是否真实共现”。
- **Hierarchical softmax**：把“全词表多分类”改成“树路径上的多个二分类”，省计算。
- **Attention**：不是直接理解句子，而是在每个位置计算“我应该看哪些其他 token”。
- **Q/K/V**：Query 像问题，Key 像索引，Value 才是真正被加权汇总的信息。
- **Encoder vs decoder**：encoder 适合理解整段输入；decoder 适合一步步生成输出。
- **Masked attention**：生成第 $t$ 个 token 时不能偷看未来 token。
- **LM head**：Transformer block 输出的是向量，LM head 才把向量变成词表概率。
- **LoRA**：大模型主体不动，只训练低秩小矩阵 $A,B$ 来表示参数改变量。
- **HMM vs CRF**：HMM 解释“词和标签怎么生成”；CRF 直接判断“给定词序列，标签序列是什么”。
- **Viterbi vs beam search**：Viterbi 是结构化 tagging 的精确动态规划；beam 是生成任务里的近似搜索。
- **Token classification vs sequence classification**：NER/POS 是每个 token 一个标签；sentiment 是整句一个标签。
- **BLEU clipped precision**：主要防止模型重复输出同一个正确词刷分。
- **Top-p sampling**：不是选概率最高的 $p$ 个词，而是选累计概率达到 $p$ 的候选集合。
- **Extractive vs abstractive**：extractive 直接摘原句；abstractive 会重新生成表达。
- **Intrinsic vs extrinsic hallucination**：intrinsic 是和原文矛盾；extrinsic 是原文没支持。
- **Cohen's kappa**：不是单纯看一致率，还要扣除“随机猜也可能一致”的部分。
