...
Finally, the method controls edit aggressiveness via _min_replacements and _max_replacements, based on CEFR gap and sentence candidate count. It selects top replacements in priority order (change_candidates, then backups, then forced candidates), applies them by token index, and reconstructs the sentence while preserving original whitespace and punctuation.

Overall, this design aims to keep substitutions meaning-preserving and fluent, while still making measurable CEFR-level shifts.